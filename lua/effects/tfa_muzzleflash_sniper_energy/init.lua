...
function EFFECT:Init(data)
	...
	local mp = emitter:Add("effects/muzzleflashX_nemole_w", pos)
	if mp then
		...
		mp:SetEndSize(12 * (halofac * 0.8 + 0.2), 0, 1)
		local r = math_Rand(-10, 10) * 3.14 / 180
		mp:SetRoll(r)
		mp:SetRollDelta(r / 5)
		mp:SetColor(97, 218, 255)
	...
	do
		local p = emitter:Add("effects/scotchmuzzleflashw", pos)
		if p then
	...
	for i = 0, 5 do
		...
		if p then
			...
			p:SetEndSize(math_max(5.4 - 0.55 * i, 1) * 1.25)
			p:SetRoll(math_rad(math_Rand(0, 360)))
			p:SetRollDelta(math_rad(math_Rand(-40, 40)))
			p:SetColor(97, 218, 255)
			p:SetLighting(false)
	...
	local sparkcount = math_random(8, 12)
	for i = 0, sparkcount do
		local p = emitter:Add("effects/yellowflare", pos)
		if p then
			p:SetVelocity((VectorRand() + Vector(0, 0, 0.3)) * 60 * Vector(0.8, 0.8, 0.6) + dir * math_Rand(80, 120) + 1.15 * addVel)
			p:SetLifeTime(0)
