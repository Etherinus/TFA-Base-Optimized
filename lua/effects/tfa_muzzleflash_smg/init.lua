...
function EFFECT:Init(data)
	...
	if mp then
		...
		mp:SetEndAlpha(0)
		mp:SetStartSize(2 * (halofac * 0.8 + 0.2), 0, 1)
		mp:SetEndSize(4 * (halofac * 0.8 + 0.2), 0, 1)
	...
	for i = 0, 5 do
		...
		if p then
			...
			p:SetEndAlpha(0)
			p:SetStartSize(math_max(5.4 - 0.55 * i, 1) * 0.35)
			p:SetEndSize(math_max(5.4 - 0.55 * i, 1) * 0.65)
			p:SetRoll(math_rad(math_Rand(0, 360)))
			p:SetRollDelta(math_rad(math_Rand(-40, 40)))
			p:SetColor(255, 218, 97)
			p:SetLighting(false)
	...
	local sparkcount = math_random(1, 2)
