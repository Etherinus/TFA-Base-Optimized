...
function EFFECT:Init(data)
	...
	for i = 0, 5 do
		local p = emitter:Add("particles/flamelet" .. math_random(1, 5), pos + (dir * 0.5 * i))
		if p then
			p:SetVelocity((dir * 7 * i) + 1.05 * addVel)
			p:SetLifeTime(0)
			p:SetDieTime(0.125)
			p:SetStartAlpha(math_Rand(200, 255))
			p:SetEndAlpha(0)
			p:SetStartSize(math_max(5.4 - 0.9 * i, 1) * 0.3)
			p:SetEndSize(math_max(5.4 - 0.9 * i, 1) * 0.5)
			p:SetRoll(math_rad(math_Rand(0, 360)))
			p:SetRollDelta(math_rad(math_Rand(-40, 40)))
			p:SetColor(255, 218, 97)
			p:SetLighting(false)
	...
	for i = 0, sparkcount do
		...
		if p then
			p:SetVelocity((VectorRand() + Vector(0, 0, 0.3)) * 15 * Vector(0.8, 0.8, 0.6) + dir * math_Rand(45, 60) + 1.15 * addVel)
			p:SetLifeTime(0)
